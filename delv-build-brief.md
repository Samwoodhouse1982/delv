...
| Content | Typed data files in `src/content/` (one `.ts` per page) | Sam edits copy often. Copy should never require touching a component. |
...
delv-site/
  ...
  src/
    ...
    components/
      ...
      ProblemList.astro
      QA.astro
      Note.astro                  # yellow left-rule callout
      CtaBand.astro
      ClaimDemo.astro             # hero what/why device
      ContactForm.astro
    content/
      home.ts  approach.ts  startups.ts  how-we-work.ts  who-we-are.ts  contact.ts
...
Nav labels stay exactly as the prototype: What we do · For startups · How we work · Who we are · [Start a conversation]. Note the prototype's internal route was `/approach`; the public URL is `/what-we-do` to match the label.
...
**No gradients.** The palette was supplied with gradient variants; they are not used and should not be introduced. Flat colour and hairlines are the system.
...
All copy is final and lives in `reference/prototype.html`. Move it into `src/content/*.ts` as typed objects. Rules:
...
- The `Privacy` footer link currently has no href
...
**For startups** — hero → "Three moments when founders call us" stage list → dim band with three stage-of-company pillars → "What you end up with" detail list plus the yellow-rule note → dark CTA band ending on the Value Audit.
...
1. Client-side validation on submit. Inline errors in the interface's voice — say what is missing and how to fix it. No apologies, no red banner at the top.
...
- Open Graph and Twitter card tags on every page, with a static OG image per page in `public/og/`. Build the OG images in the site's own visual language: ink background, `delv.` logotype, page heading in Archivo, the tick rule. Do not generate them at request time.
...
**Accessibility**
- Keyboard-navigable end to end, with a visible focus ring on every interactive element (2px ink, 3px offset; yellow on dark bands).
- Working skip link.
- Every page has exactly one `h1` and no skipped heading levels.
- Body text meets WCAG AA (4.5:1). The de-emphasised claim line in the hero is large text and must meet 3:1 — it is `#7E8781` on `#F4F4F0` in the prototype; do not lighten it.
...
- Mobile nav: the prototype's toggle pattern, `aria-expanded` maintained, closes on route change and on Escape.
...
1. Scaffold Astro, install deps, set up the repo structure, self-host the fonts.
...
4. Move all copy into `src/content/*.ts` and render the six pages. **Checkpoint: preview deploy for Sam.**
...
8. Accessibility and Lighthouse passes; fix to the criteria in §10.
